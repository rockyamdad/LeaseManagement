...
class KhatianActionMenuManager
{
    ...
    protected function getEntryOperatorMenu()
    {
        ...
        if (in_array($this->khatianLog->getKhatianStatus(), array('DRAFT', 'CORRECTION_REQUIRED'))) {
            ...
            $khatianVersion = $this->khatian->getLastVersion();
        ...
        } elseif ($this->khatianLog->getKhatianStatus() == 'READY_FOR_VERIFICATION' && !$this->khatianLog->isLocked()) {
            ...
            $html .= '<a href="'.$this->router->generate('khatian_move_to_draft', array('id' => $this->khatianLog->getId())).'" class="btn blue btn-sm" title="খসড়াতে প্রেরণ "><i class="fa fa-undo"></i></a>';
        }

        if ($this->khatianLog->getKhatianStatus() == 'DRAFT' && $this->khatianLog->getVerifier() == null) {
            $html .= '<a href="'.$this->router->generate('khatian_pages', array('id' => $this->khatianLog->getId())).'" class="btn blue btn-sm" title="দেখুন " ><i class="fa fa-file-o"></i></a>';
            //$html .= '<a href="'.$this->router->generate('delete_khatian', array('id' => $this->khatian->getId())).'" class="btn red btn-sm confirm" title="খতিয়ান মুছুন"><i class="fa fa-times"></i></a>';
        }
    ...
    private function getKhatianViewLink($status)
    {
        ...
        if ($this->khatianLog->getKhatianStatus() == $status && !$this->khatian->isLocked() ||
            (
                $lockedBy && $lockedBy->getId() == $this->loggedInUser->getId() && $this->khatian->isLocked()
            )
        ) {
            $html .= '<a href="'.$this->router->generate('workflow_khatian_pages', array('id' => $this->khatianLog->getId())).'" class="btn blue btn-sm" title="খাতিয়ান দেখুন "><i class="fa fa-file-o"></i></a>';
        }

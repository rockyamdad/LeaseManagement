...
use PorchaProcessingBundle\Form\ServiceRequestCaseCopyType;
...
class CaseCopyRequestController extends BaseRequestController
{
    /**
     * @Template("@PorchaProcessing/Service/CaseCopy/application_create.html.twig")
     * @JMS\Secure(roles="ROLE_CASE_COPY_REQUEST_ENTRY")
     */
    public function createAction(Request $request)
    {
        if (!$this->isServiceEnabled()) {
            ...
            return $this->redirect($this->generateUrl('case_copy_list'));
        }
        
        $officeType = $this->getUser()->getOffice()->getType();
        $serviceRequest = new ServiceRequest();
        $serviceRequestCaseCopy = new ServiceRequestCaseCopy();

        $serviceType = 'CASE_COPY';
        $serviceRequest->setType($serviceType);
        $this->setPorchaRequestPostalAddressFromSesion($serviceRequest);
        $this->determineWhatCanBeEditServiceRequest($serviceRequest, $serviceRequestCaseCopy);
        $serviceRequestCaseCopy->setServiceRequest($serviceRequest);

        $form = $this->createForm(new ServiceRequestCaseCopyType(null, $officeType), $serviceRequestCaseCopy);
        if($officeType=='UDC'){
            $thisOffice = $this->getUser()->getOffice();
            /**@var Office $thisOffice*/
            $officeSetting = $this->getSingleServiceFee($thisOffice->getParent(), $serviceRequest);
        }else{
            $officeSetting = $this->getSingleServiceFee($this->getUser()->getOffice(), $serviceRequest);
        }
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($form->isValid()) {
                ...
                if($requestCount >= $applicationLimitPerDay){
                    ...
                    return $this->redirect($this->generateUrl('case_copy_create'));
                }


                $serviceRequest->setOffice($this->getUser()->getOffice());
                $serviceRequest->setRequestFrom('DIRECT');
                $this->getDoctrine()->getRepository('PorchaProcessingBundle:ServiceRequestCaseCopy')
                    ->setOfficeManager($this->get('app.service.office_manager'))
                    ->createCaseCopy($serviceRequestCaseCopy, $officeSetting, $serviceType);

                $this->addFlash('success', $this->get('translator')->trans('Case Copy Request Submitted Successfully'));
                ...
                return $this->redirect($this->generateUrl('case_copy_list', array('serviceType' => $serviceType)));
            }
        }
    ...
    {
        return parent::getSingleServiceFee($office, $serviceRequest, 'CASE_COPY');
    }
    ...
    public function editAction(Request $request, ServiceRequest $serviceRequest)
    {
        ...
        if ($serviceRequest->getStatus() == 'DELIVERED') {
            ...
            return $this->redirect($this->generateUrl('case_copy_list', array('serviceType' => strtolower($serviceRequest->getType()))));
        }

        $serviceRequestCaseCopy = $this->getDoctrine()
            ->getRepository('PorchaProcessingBundle:ServiceRequestCaseCopy')
            ->findOneBy(array('serviceRequest' => $serviceRequest));
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($form->isValid()) {
                ...
                $this->getDoctrine()->getRepository('PorchaProcessingBundle:ServiceRequestCaseCopy')
                    ->setOfficeManager($this->get('app.service.office_manager'))
                    ->saveCaseCopy($serviceRequestCaseCopy, $officeSetting, 'CASE_COPY');

                $this->addFlash('success', $this->get('translator')->trans('Case Copy Request Updated Successfully'));
    ...
    /**
     * @JMS\Secure(roles="ROLE_CASE_COPY_REQUEST_MANAGE")
     */
    public function listAction(Request $request, $serviceType = 'porcha_request')
    {
        ...
        $serviceRequest = $this->getDoctrine()->getRepository('PorchaProcessingBundle:ServiceRequestCaseCopy')->getList($requestAll, $serviceType);
        ...
        $template = 'PorchaProcessingBundle:Service/CaseCopy:application_list.html.twig';
        if ($request->isXmlHttpRequest()) {
            $template = 'PorchaProcessingBundle:Service/CaseCopy:application_list_sub.html.twig';
        }
     ...
     * @JMS\Secure(roles="ROLE_CASE_COPY_REQUEST_MANAGE")
     */
    public function printAction(Request $request, ServiceRequest $serviceRequest)
    {
    ...
    public function updateDeliveryStatusAction(ServiceRequest $serviceRequest)
    {
        ...
        if ($serviceRequest->getStatus() == 'PENDING' && $this->isGranted('ROLE_START_SERVICE_REQUEST')) {
            $message = 'Case copy is now ready for processing';
            ...
            $message = 'Case copy is now ready for delivery';
    ...
    public function saveCaseCopyAction(Request $request){
        ...
        $citizen = $this->getDoctrine()->getRepository('PorchaProcessingBundle:ServiceRequestCaseCopy')
            ->setOfficeManager($this->get('app.service.office_manager'))
            ->createService($data);

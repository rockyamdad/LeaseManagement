$(function() {
    ...
    function calculateFee(){
        ...
        $('#khatian-list').find('tr').each(function(index, tr){
            var row = $(tr);
            var surveyId = row.find('.survey_selector').val();
        ...
        totalCell.text(AppUtil.banglaToEnglish(String(courtFee + deliveryFee)) + '/=');
    }
    ...
    $('.applicant-address').find('input, select').blur(function(){
    ...
    $('form[name=porchaprocessingbundle_servicerequest]').validate({
        rules: {
            'porchaprocessingbundle_servicerequest[district]': {
                required: {
                    depends: function (element) {
                        return isPostalAddressRequire();
                    }
                }
            }
        },
